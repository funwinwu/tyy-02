...
class Mage_Page_Block_Template_Links extends Mage_Core_Block_Template
{
        ...
        $position=null, $liParams=null, $aParams=null, $beforeText='', $afterText='')
    {
		...
		$link_url = ($prepare ? $this->getUrl($url, (is_array($urlParams) ? $urlParams : array())) : $url);
        $link = new Varien_Object(array(
			...
			'is_current'    => $this->isCurrent( $link_url ),
        ));
	...
	/**
	*check if the link is visited currently.
	*@return boolen
	...
	**/
	public function isCurrent( $url )
	{
		...
		if( $url == '/' ){
			if( is_object( Mage::app()->getFrontController()->getData('action') ) && get_class( Mage::app()->getFrontController()->getData('action') ) == 'Mage_Cms_IndexController' )
				return true;
			else return false;
		}else
			return eregi( $url.'$',$current_uri );
	}
}
